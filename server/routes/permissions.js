...
router.get('/permissions', verify, async (req, res) => {
    try {
        const userPermission = await UserPermission.find({userId: req.user._userId});
        if (!userPermission) {
...
router.get('/resourcePermission', verify, async (req, res) => {
    try {
        const userPermission = await UserPermission.findOne({resourceId: req.body.resourceId});
        if (!userPermission) {
            return res.status(400).send('resource is not available for user');
        }
        res.status(200).send(req.body.permission)
...
router.post('/upsertPermission', verify,  async (req, res) => {
    ...
    if (!resource) {
        return res.status(400).send('Cannot register permission, user does not own resource');
    }
    ...
    } else { // Create a new user permission
