...
router.post('/registerResource', verify, async (req, res) => {
    ...
    const resourceNameExists = await Resource.findOne({resourceName: req.body.resourceName});
    if(resourceNameExists) return res.status(400).send('Resource name already exists');
...
//Get all resources
router.get('/resources', verify, async (req, res) => {
    try {
